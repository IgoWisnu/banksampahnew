...

{
    ...
    public function exceltransaksi()
    {
        ...
        $sheet->mergeCells('A1:G1'); // Ubah merge sampai kolom G
        ...
        // ==== DESAIN TABEL HEADER (TAMBAH MARGIN) ====
        $headers = ['No', 'Tanggal', 'Nama Nasabah', 'Setor (Debit)', 'Margin', 'Tarik (Kredit)', 'Keterangan Sampah'];
        $columnLetter = 'A';
        ...
        foreach ($laporan->result_array() as $row) {
            ...
            $sheet->setCellValue('E' . $rowNum, $row['margin']); // Kolom Baru: Margin
            ...
            $sheet->setCellValue('G' . $rowNum, $keterangan);
            ...
            $sheet->getStyle('A' . $rowNum . ':G' . $rowNum)->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN)->getColor()->setRGB('EEEEEE');
        ...
        $sheet->setCellValue('E' . $rowNum, $total_margin);
        $sheet->setCellValue('F' . $rowNum, $total_tarik);
        ...
        $sheet->getStyle('A' . $rowNum . ':G' . $rowNum)->applyFromArray($totalStyle);
